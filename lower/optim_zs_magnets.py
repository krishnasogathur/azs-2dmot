...
This script is configured for the LOWER MODEL N=6 Stage-2 optimisation
(ZS not yet run for N=6 lower model — see optimization-results.txt).
It uses make_B_field (N=6 adjustable) from magnets.py.
For the N=5 lower-model ZS result, see optimization-results.txt directly.

2DMOT params are held fixed at the N=6 lower-model 2DMOT-only optima
(Δ_2D ≈ 2.58 Γ, P_2D ≈ 47.5 mW, δx ≈ 2.76 mm — from raw-optim-logs/).

Search space:
  Delta0_zs  ∈ [3.3, 4.9] Γ
  power_zs   ∈ [95, 130] mW
  (delta_x currently fixed at 2.76 mm — uncomment SPACE entry to re-optimise)

Fixed:
  Delta0_2DMOT = 2.58 Γ  (N=6 lower-model 2DMOT-only optimum)
  power_2DMOT  = 47.5 mW (N=6 lower-model 2DMOT-only optimum)
...
DELTA0_2DMOT  = 2.58     # fixed from optim_2dmot
POWER_2DMOT_W = 47.5e-3  # fixed from optim_2dmot
DELTA_X_FIXED = 2.76e-3  # fixed magnet offset (m); was a search param, now fixed

SPACE = [
    Real(3.3,    4.9,    name="Delta0_zs"),
    Real(95e-3, 130e-3,  name="power_zs"),
...
if __name__ == "__main__":
    ...
    print(f"  Search:  ΔZS ∈ [3.3,4.9]Γ   P_ZS ∈ [95,130]mW")
