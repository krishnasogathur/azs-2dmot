"""
Semiclassical force model — shared saturation (lower bound on scattering rate).

Total saturation in the denominator is the raw sum of all beam×polarization components:
    s_tot = Σ_i s_i * I_q(i)
No detuning weighting in the total. Detuning enters only in the per-beam rate formula:
    R_q = (Γ/2) * s_i*I_q / (1 + s_tot + (2Δ_q/Γ)²)

This is exact when all beams are equally detuned (shared saturation model) and gives
a lower bound on the true scattering rate in the multi-beam, multi-detuning case.
Compare with upper/physics.py which uses the s_eff model (upper bound).
"""
...
def scattering_rate_multi(delta_list, s_list, Gamma=Gamma):
    # diagnostic helper illustrating the lower-bound formula; not called by the integrator
    # (state_propagate_3D computes rates inline with full polarization decomposition)
    s_tot = sum(s_list)
    return [
        (Gamma / 2) * s_i / (1 + s_tot + (2 * delta_i / Gamma)**2)
        for s_i, delta_i in zip(s_list, delta_list)
    ]
...
def sample_effusive_velocity_3D(T=320+273, m=m_actual, size=1, a_cap=0.5e-3, L_cap=20e-3):
    """Maxwell-Boltzmann effusive flux distribution, collimated by capillary."""
    kB = 1.380649e-23
    a  = m / (2 * kB * T)
...
def sample_initial_positions_3D(size=1, n_cap=133, a_cap=0.5e-3, z0=z0):
    """Uniform random positions over the capillary array cross-section."""
    ...
    return np.column_stack((
        np.zeros(size),   # x = 0 (on-axis approximation)
        np.zeros(size),
        np.full(size, z0),
    ))
...
def polarization_weights(k_beam, B):
    """
    Decompose a circularly polarized beam into σ+/π/σ− components
    relative to the local quantization axis set by B.

    ZS beams (not along 45°): worst-case assumption of 50/50 σ+/σ−.
    2DMOT beams (±45° in x-z): RCP for x=-z diagonal, LCP for x=z diagonal,
    consistent with the field configuration that produces restoring forces.
    """
    if np.abs(k_beam[0]) != np.abs(k_beam[2]):
        # ZS beam — B not well defined along entire path, use 50/50
        return {+1: 0.5, 0: 0.0, -1: 0.5}

    factor = 1
    if k_beam[0] * k_beam[2] > 0:
        factor *= -1   # x=z diagonal needs LCP for restoring force
...
def state_propagate_3D(
    ...
    # total saturation: raw sum, no detuning weighting (shared saturation / lower bound)
    s_tot = 0.0
    for Delta0_i, k_i, s_i in zip(Delta0, k_list, s_list):
        weights = polarization_weights(k_i, B)
        for I_q in weights.values():
            s_tot += s_i * I_q
    ...
    for Delta0_i, k_i, s_i in zip(Delta0, k_list, s_list):
        ...
        for q, I_q in weights.items():
            ...
            R_q      = scattering_rate_delta(s_q, s_tot, Delta_q)
            F       += hbar * k_i * R_q
            R_total += R_q
