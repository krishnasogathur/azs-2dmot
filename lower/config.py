...
# natural units: energy in hbar*Gamma, length in sqrt(hbar/m/Gamma), time in 1/Gamma
hbar = 1.0
Gamma = 1.0
m = 1.0

hbar_actual  = 1.0545718e-34
Gamma_actual = 2 * np.pi * 29.1e6   # Yb-174 imaging linewidth (Hz)
...
x_scale        = (hbar_actual / (m_actual * Gamma_actual))**0.5 * 1e9   # nm/natural_length
lambda_actual  = 399   # nm
lambda_natural = lambda_actual / x_scale
k_399_natural  = 2 * np.pi / lambda_natural
v_scale        = x_scale * 1e-9 * Gamma_actual   # natural_velocity -> m/s

z0 = -25e-2   # atom source position (m); oven ~25 cm upstream

# -----------------------------------------------------------------------
# ZS beams — two counter-propagating beams at ±20° from z-axis in x-z plane
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# 2DMOT beams — four beams at ±45° in x-z plane, retroreflected
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# Saturation parameters and beam geometry
# -----------------------------------------------------------------------
I_sat        = 60      # mW/cm^2
CF25_diameter = 2.3e-2
CF40_diameter = 3.9e-2

zs_power  = 90e-3    # W per ZS beam  (lower-model optimum ~85–94 mW; upper/config.py uses higher power)
w0_zs     = 2e-2 / 2
...
power_2DMOT = 37.5e-3   # W per beam  (lower-model N=5 optimum ~37–38 mW)
w0_2DMOT_1  = 1.5e-2 / 2
